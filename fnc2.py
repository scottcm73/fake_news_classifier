...
def train_model(X_train):
    ...
    hash_text=HashingVectorizer(ngram_range=(3, 7), analyzer="word", alternate_sign=False)
    ...
    return
class model_test():
    # The model test class has the vectorize and test_model methods using Hashing Vectorizer
    def __init__(self):
        return
    ...
    def test_model(self, X_test):
        ...
        LR3 = LogisticRegression(random_state=0, solver='lbfgs', multi_class='ovr').fit(X1, y_train)
        LR2 = LogisticRegression(random_state=0, solver='lbfgs', multi_class='ovr').fit(X2, y_train)
        LR1 = LogisticRegression(random_state=0, solver='lbfgs', multi_class='ovr').fit(X3, y_train)

       
        
     
        X4 = self.vectorize(X_test)
        author_predict=LR1.predict(X4)
        # author_predict = np.asarray(test_predict, dtype=np.float64, order='C')

        X5 = self.title_vector
        title_predict=LR2.predict(X5)
        # title_predict = np.asarray(test_predict2, dtype=np.float64, order='C')
        self.title_predict=title_predict
        X6 = self.text_vector

        text_predict=LR3.predict(X6)
...
arr_train=train_df.values.tolist()
...
clean_news=clean_news_arr(arr_train)
# print('clean_news')
# print(clean_news)



clean_news_arr=np.array(clean_news)



X=clean_news_arr[:,1:4]
y=clean_news_arr[:, 4]
...
print(classification_report(y_true, y_pred1, labels=None))
print(classification_report(y_true, y_pred2, labels=None))
print(classification_report(y_true, y_pred3, labels=None))
